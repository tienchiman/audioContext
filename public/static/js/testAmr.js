...
class AudoApi {
    ...
    init(href, onPlaying) {
        ...
        this.start = () => {
            ...
            if(!this.context || /^closed$/.test(this.context.state)){
                initEnv(envim => {
                    this.isPlaying = true;
                    envim.ctx.resume();
                    onPlayInterval(envim.duration);
...
class Page {
    ...
    methods() {
        return {
            ...
            onSomeAudoPlaying(index, duration, current,isPlaying) {
                current = isPlaying ? current : 0;
                this.setState(index, {progress: `${current / duration * 100}%`, state: isPlaying? 'play': 'stop'});
            }
